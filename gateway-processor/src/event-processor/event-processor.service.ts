...
export class EventProcessorService implements OnModuleInit, OnModuleDestroy {
    ...
    constructor(
      ...
      private readonly metricsService: MetricsService,
        ...
        this.MAX_RETRIES = parseInt(this.configService.getOrThrow('OUTBOX_MAX_RETRIES'));
    }
    ...
    private async processBatch() {
        ...
        if (failures.length) {
            const ops = failures.map(f => {
                const nextStatus = f.retries >= this.MAX_RETRIES ? OutboxStatus.FAILED : OutboxStatus.PENDING;
