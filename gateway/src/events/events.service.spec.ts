...
describe('EventsService', () => {
	...
	describe('processRequest', () => {
		it('should return 503 when shutting down', async () => {
			...
			await service.processRequest(mockRequest as Request, mockResponse as Response);
		...
		it('should handle stream request when not shutting down', async () => {
			const handleStreamRequestSpy = jest.spyOn(service as any, 'handleStreamRequest')
				.mockResolvedValue(undefined);

			await service.processRequest(mockRequest as Request, mockResponse as Response);

			expect(handleStreamRequestSpy).toHaveBeenCalledWith(mockRequest, mockResponse);
		});
	...
	describe('handleStreamRequest', () => {
		it('should create and configure worker', async () => {
			...
			const handleStreamRequest = (service as any).handleStreamRequest.bind(service);
			await handleStreamRequest(mockRequest, mockResponse);

			expect(Worker).toHaveBeenCalledWith(expect.stringContaining('batch-processing.worker.js'));
		...
		it('should handle pipeline error event', async () => {
			...
			await expect(handleStreamRequest(mockRequest, mockResponse)).rejects.toBeUndefined();
	...
	describe('processWithWorker', () => {
		...
		it('should handle worker error', async () => {
			...
			await expect(processWithWorker(mockWorker, events)).rejects.toThrow('Worker error');
			expect(mockWorker.postMessage).toHaveBeenCalledWith(events);
		});
	...
	describe('saveBatch', () => {
		it('should save batch successfully', async () => {
			...
			(prismaService.outboxEvent.createMany as jest.Mock).mockResolvedValue({count: 2});
		...
		it('should handle database error', async () => {
			...
			(prismaService.outboxEvent.createMany as jest.Mock).mockRejectedValue(dbError);
			...
			expect(consoleSpy).toHaveBeenCalledWith('Error during saving to DB', dbError);
		...
		it('should always increment processed counter', async () => {
			...
			(prismaService.outboxEvent.createMany as jest.Mock).mockRejectedValue(new Error('DB Error'));
	...
	describe('Integration scenarios', () => {
		it('should handle complete request flow', async () => {
			...
			(Worker as any as jest.Mock).mockReturnValue(mockWorker);
			(prismaService.outboxEvent.createMany as jest.Mock).mockResolvedValue({count: 1});

			const processWithWorkerSpy = jest.spyOn(service as any, 'processWithWorker')
